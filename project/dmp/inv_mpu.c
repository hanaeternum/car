 ...
 * rt_thread_mdelay(unsigned long num_ms)
...
#define rt_thread_mdelay    msp430_rt_thread_mdelay
...
/* rt_thread_mdelay is a function already defined in ASF. */
...
static int accel_self_test(long *bias_regular, long *bias_st)
{
    ...
    for(jj = 0; jj < 3; jj++) {
        ...
        if (st_shift[jj]) {
            ...
            if (abs(st_shift_var) > test.max_accel_var)
                result |= 1 << jj;
